import { decode, decodeString, encodeToString, } from "../../../../std@0.100.0/encoding/hex.js";
...
export function toScriptRef(script) {
    switch (script.type) {
        ...
        case "PlutusV1":
            return C.ScriptRef.new(C.Script.new_plutus_v1(C.PlutusScript.from_bytes(fromHex(script.script))));
        case "PlutusV2":
            return C.ScriptRef.new(C.Script.new_plutus_v2(C.PlutusScript.from_bytes(fromHex(script.script))));
        default:
            throw new Error("No variant matched.");
    }
}
...
export function applyParamsToScript(plutusScript, params, shape) {
    const p = shape
        ? Data.castTo(params, shape).fields
        : params;
    return toHex(C.apply_params_to_plutus_script(C.PlutusList.from_bytes(fromHex(Data.to(p))), C.PlutusScript.from_bytes(fromHex(plutusScript))).to_bytes());
}
