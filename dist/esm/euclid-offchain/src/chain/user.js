...
export class User {
    ...
    address, paymentKeyHash) {
        ...
        if (address) {
            ...
            const paymentKeyHash_ = KeyHash.fromCredential(addressDetails.paymentCredential);
            if (paymentKeyHash) {
    ...
    static async generateWith(lucid, allAssets) {
        ...
        user.balance = PositiveValue.genOfAssets(allAssets.boundedSubset(1n)).normedPlus(forFeesEtc);
        return user;
    }
    // for propertytesting
