...
export class DiracUtxo {
  ...
  ${tt})`;
    ...
    Object.defineProperty(this, "openingTx", {
      ...
      writable: true,
      value: (tx, contract) => {
        const diracDatum = this.peuclidDatum.pconstant(
          new DiracDatum(this.dirac),
        );
    ...
    Object.defineProperty(this, "swappingsFor", {
      ...
      value: (
        user,
        paramUtxo,
        sellable_, // subset of pool-assets. NOTE: Empty if infinite for any asset, -1 if infinite for a specific asset
        buyingAsset,
      ) => {
        ...
        param.assets.forEach((asset) => {
          ...
          if ((sellable === undefined || sellable != 0n) && spotSelling > 0n) {
            while (true) {
              ...
              const maxSelling = sellable && sellable > 0n
                ? min(sellable, d)
                : d;
              if (maxSelling > 0n) {
        ...
        sellableAssets.forEach((sellingAsset) => {
          ...
          buyableAssets.forEach((buyingAsset) => {
            ...
            if (maxSwapA0 < spotSelling) {
              ...
              const buyingJumpSize = Number(
                param.jumpSizes.amountOf(buyingAsset),
              );
              const sellingJumpMultiplier = (sellingJumpSize + 1) /
                sellingJumpSize;
              const buyingJumpMultiplier = (buyingJumpSize + 1) /
                buyingJumpSize;
              ...
              while (maxSwapA0 < spotSelling) {
                if (maxSellingA0 <= maxBuyingA0) {
                  ...
                  const spotSelling_ = Math.floor(
                    sellingAnchor *
                      (sellingJumpMultiplier ** Number(expSelling)),
                  );
                  ...
                  spotBuying = BigInt(
                    Math.floor(
                      buyingAnchor *
                        (buyingJumpMultiplier ** Number(expBuying)),
                    ),
                  );
