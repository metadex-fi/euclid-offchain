    ...
    constructor(pelem, length) {
        ...
        Object.defineProperty(this, "plift", {
            ...
            value: (l) => {
                ...
                assert(!this.length || this.length === BigInt(l.length), `plift: wrong length - ${this.length} vs. ${l.length}`);
        ...
        Object.defineProperty(this, "pconstant", {
            ...
            value: (data) => {
                ...
                assert(!this.length || this.length === BigInt(data.length), `pconstant: wrong length`);
        ...
        Object.defineProperty(this, "showData", {
            ...
            value: (data, tabs = "", maxDepth) => {
                if (maxDepth !== undefined && maxDepth <= 0n)
                    return "List [ … ]";
                assert(data instanceof Array, `PList.showData: expected Array, got ${data}`);
                const tt = tabs + t;
                const ttf = tt + f;
                return `List [
${data.map((d) => `${ttf}${this.pelem.showData(d, ttf, maxDepth ? maxDepth - 1n : maxDepth)}`).join(",\n")}
${tt}]`;
            }
        });
