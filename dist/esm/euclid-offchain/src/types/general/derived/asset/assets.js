...
import { boundedSubset, gMaxLength, max, nonEmptySubSet, randomChoice, randomSubset, } from "../../../../utils/generators.js";
    ...
    constructor(assets = ccysTkns.anew) {
            ...
            value: (tabs = "") => {
                ...
                for (const [currency, tokens] of this.assets) {
                    ...
                    for (const token of tokens) {
                        tkns.push(`${ttff}${token.name === "" ? symbol === "" ? "lovelace" : "_" : token.name}`);
                    }
                    ccys.push(tkns.join(",\n"));
                }
                return ccys.join(`\n`);
        ...
        Object.defineProperty(this, "insert", {
            ...
            value: (asset) => {
                ...
                assert(!ownTkns.some((own) => own.name === token.name), `${asset} already in ${this.show()}`);
                ownTkns.push(token);
        ...
        Object.defineProperty(this, "intersect", {
            ...
            value: (assets) => {
                ...
                for (const [ccy, ownTkns] of this.assets) {
                    ...
                    if (otherTkns) {
                        const sharedTkns = ownTkns.filter((own) => otherTkns.some((other) => other.name === own.name));
...
Object.defineProperty(Assets, "generate", {
    ...
    value: (minLength = 0n, maxLength) => {
        const assets = PMap.genKeys(PAsset.ptype, newGenInRange(minLength, maxLength ?? max(minLength, gMaxLength))());
