...
import { genNonNegative, gMaxLength, maybeNdef, } from "../../../../utils/generators.js";
    ...
    constructor(showKey) {
        ...
        Object.defineProperty(this, "show", {
            ...
            value: (showValue, tabs = "") => {
                ...
                const ttf = tt + f;
                return `AssocMap {
      ${[...this.inner.values()].map(([key, value]) => `${ttf}${this.showKey(key, ttf)} => ${showValue(value, ttf)}`).join(",\n")}
      ${tt}}`;
            }
        });
    }
    ...
    constructor(pkey, pvalue, size, keys, dataKeys) {
        ...
        Object.defineProperty(this, "plift", {
            ...
            value: (m) => {
                ...
                m.forEach((value, key) => {
                    ...
                    assert(!this.dataKeys || Data.to(this.dataKeys[i++]) === Data.to(key), `PMap.pconstant: wrong key of ptype
        ...
        Object.defineProperty(this, "pconstant", {
            ...
            value: (data) => {
                ...
                assert(!this.size || this.size === BigInt(data.size), `AssocMap.pconstant: wrong size: ${this.size} vs. ${data.size} of ${this.showData(data, "", maxShowDepth)}`);
                const m = new Map();
                let i = 0;
                data.forEach((value, key) => {
                    const k = this.pkey.pconstant(key);
                    assert(!this.dataKeys ||
                        Data.to(this.dataKeys[i++]) ===
                            Data.to(k), `PMap.plift: wrong key`);
        ...
        Object.defineProperty(this, "genData", {
            ...
            value: () => {
                if (this.keys) {
                    ...
                    const size = this.size ? this.size : genNonNegative(BigInt(Math.min(Number(gMaxLength), this.pkey.population)));
        ...
        Object.defineProperty(this, "showData", {
            ...
            value: (data, tabs = "", maxDepth) => {
                if (maxDepth !== undefined && maxDepth <= 0n)
                    return "AssocMap { … }";
                assert(data instanceof AssocMap, `PMap.showData: expected AssocMap, got ${data}`);
                const tt = tabs + t;
                const ttf = tt + f;
                return `AssocMap {
${[...data.entries()].map(([key, value]) => `${ttf}${this.pkey.showData(key, ttf, maxDepth ? maxDepth - 1n : maxDepth)} => ${this.pvalue.showData(value, ttf, maxDepth ? maxDepth - 1n : maxDepth)}`).join(",\n")}
${tt}}`;
            }
        });
...
${ttf}pvalue: ${this.pvalue.showPType(ttf, maxDepth ? maxDepth - 1n : maxDepth)},
${ttf}size?: ${this.size},
${ttf}keys?: ${keys}
${tt})`;
            }
        ...
        if (keys) {
            ...
            assert(Number(size) <= pkey.population, `PMap: not enough keys for size ${size} in\n${pkey.showPType()}`);
...
${t}pkey: ${pkey.showPType(t)}`);
        ...
        if (size) {
            assert(size <= maxKeys, `PMap.genKeys: size too big: ${Number(size)} vs. ${maxKeys}`);
            while (keys.length < size)
                genKey();
        }
        else {
            const size_ = genNonNegative(BigInt(Math.min(Number(gMaxLength), maxKeys)));
    ...
    static genMap(pkey, pvalue, size) {
        assert(Number(size) <= pkey.population, `PMap: not enough keys for size ${Number(size)} in ${pkey.showPType()}`);
    ...
    static genPType(gen, maxDepth) {
        ...
        const size = maybeNdef(BigInt(keys?.length ??
