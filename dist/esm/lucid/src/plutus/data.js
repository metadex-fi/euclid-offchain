      ...
      switch (enumShape.dataType) {
        case "constructor": {
          if (enumShape.fields.length === 0) {
            ...
            return {
              [enumShape.title]: enumShape.fields.map((field, index) =>
      ...
      if (shape.items instanceof Array) {
        ...
        if (
          ...
          return data.fields.map((field, index) =>
            castFrom(field, shape.items[index])
          );
  ...
  switch (shapeType) {
    ...
    case "constructor": {
      ...
      const fields = shape.fields.map((field) =>
        castTo(struct[field.title], field)
      );
    ...
    case "enum": {
      ...
      switch (typeof struct) {
        case "string": {
          ...
          const enumIndex = shape.anyOf.findIndex((s) =>
        ...
        case "object": {
          ...
          const enumEntry = shape.anyOf.find((s) =>
            s.dataType === "constructor" &&
            s.title === structTitle
          );
          if (!enumEntry) {
          ...
          return new Constr(
            enumEntry.index,
            struct[structTitle].map((item, index) =>
